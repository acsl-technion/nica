...
#pragma HLS stream variable=_response_stream depth=300
...
void memcached::parse_out_payload() {
#pragma HLS pipeline enable_flush ii=4
    _kv_pairs_stream.full_progress();

    if (_response_stream.empty() || _kv_pairs_stream.full())
        return;

    memcached_raw_response response = _response_stream.read();
    memcached_key_value_pair kv;
#pragma HLS array_reshape variable=kv.value.data complete
#pragma HLS array_reshape variable=kv.key.data complete

    kv.ikernel_id = response.ikernel_id;
    kv.key.actual_size = MEMCACHED_KEY_SIZE;

    bool found = false;
    for (int i = 0; i < REPLY_SIZE; ++i) {
#pragma HLS unroll
        if (i >= 14 && i <= 14 + MEMCACHED_KEY_SIZE - 1) {
            kv.key.data[i - 14] = response.data[i];

            if ((response.data[i] == ' ' || response.data[i] == '\r') && !found) {
                found = true;
                kv.key.actual_size = i - 14;
            }
        }

        assert (kv.key.actual_size <= MEMCACHED_KEY_SIZE);

        if (i >= VALUE_POS - (MEMCACHED_KEY_SIZE - kv.key.actual_size) && i < VALUE_POS - (MEMCACHED_KEY_SIZE - kv.key.actual_size) + MEMCACHED_VALUE_SIZE) {
            kv.value.data[i - (VALUE_POS - (MEMCACHED_KEY_SIZE - kv.key.actual_size))] = response.data[i];
        }
    }

    _kv_pairs_stream.write(kv);
}

void memcached::parse_in_payload(const hls_ik::axi_data &d, char udp_header[8], memcached_key<MEMCACHED_KEY_SIZE>& key) {
#pragma HLS inline
#pragma HLS array_reshape variable=key.data complete
    for (int i = 0; i < 8; ++i) {
#pragma HLS unroll
    ...
    bool found = false;
    for (int i = 12; i < 12 + MEMCACHED_KEY_SIZE; ++i) {
#pragma HLS unroll
        const int bottom = 255 - ((i + 1) * 8 - 1), top = 255 - (i * 8);
        key.data[i - 12] = d.data.range(top, bottom);

        if ((key.data[i - 12] == ' ' || key.data[i - 12] == '\r') && !found) {
            found = true;
            key.actual_size = i - 12;
        }
    }
}
    ...
    switch (_reply_state) {
        ...
        case GENERATE_RESPONSE:
            ...
            if (_reply_word == 0) {
                ...
                d.set_data(_current_response.data + 32, _current_response.size - 32);
                d.last = 1;
            }
    ...
    switch(_out_state) {
        case METADATA:
            if (!out.metadata_input.empty() && !h2n_arb.m1.full()) {
                ...
                _response.ikernel_id = _out_metadata.ikernel_id;
                _out_state = DATA;
            }

            break;
        case DATA:
            if (_response_stream.full()) return;
...
#pragma HLS unroll
                    const int bottom = 255 - ((i + 1) * 8 - 1), top = 255 - (i * 8);
                    _response.data[i + 32 * _out_word] = d.data.range(top, bottom);
                }
                ...
                if (d.last) {
                    if (_response.data[8] == 'V') {
                        _response_stream.write(_response);
    ...
    switch (_in_state) {
        case METADATA:
            if (!_parser_metadata.empty()) {
                ...
                _parsed_request.key.actual_size = MEMCACHED_KEY_SIZE;
                _in_state = DATA;
            }
            break;
        ...
        case DATA: {
            ...
            if (!_parser_data.empty() && !requests_full) {
                ...
                if (_in_offset == 0) {
                    ...
                    parse_in_payload(d, _parsed_request.udp_header, _parsed_request.key);
                    ++_in_offset;
                }
...
#pragma HLS pipeline enable_flush ii=3
    ...
    case MEM_RESP_WAIT:
        ...
        case GET: {
            ...
            if (found.valid()) {
                pass_to_host = false;
                memcached_response resp = generate_response(mem_resp_req, found.value());
                bool tc_backpressure = !can_transmit(tc_host, id, 0, resp.size, NET);
                if (tc_backpressure) {
                    _stats_cache.write(std::make_tuple(id, STAT_HIT_TC_BACKPRESSURE));
                } else if (!_reply_metadata_stream.full() && !_reply_data_stream.full()) {
                    _reply_metadata_stream.write(mem_resp_req.metadata.reply(resp.size));
                    _reply_data_stream.write(resp);
...
#pragma HLS array_partition variable=response.data complete
    ...
    for (int i = 0; i < MEMCACHED_KEY_SIZE; ++i) {
#pragma HLS unroll
        if (i < parsed_request.key.actual_size)
            response.data[14 + i] = parsed_request.key.data[i];
    }

    int idx = 14 + parsed_request.key.actual_size;

    response.data[idx++] = ' ';
    response.data[idx++] = '0'; // Flags
    response.data[idx++] = ' ';

    // Value size
    hls_helpers::memcpy<VALUE_BYTES_SIZE>(&response.data[idx], &ASCII_MEMCACHED_VALUE_SIZE[0]);

    idx += VALUE_BYTES_SIZE;
    response.data[idx++] = '\r';
    response.data[idx++] = '\n';

    hls_helpers::memcpy<MEMCACHED_VALUE_SIZE>(&response.data[idx], &value.data[0]);

    idx += MEMCACHED_VALUE_SIZE;
